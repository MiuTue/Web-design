...
const displayTimer = () =>{
    const date = new Date();
    const currentTime =  date.toLocaleTimeString("vi-VN", { hour12: false });
...
const createAlarm = (hour, minute) =>{
    ...
    //Create ad checkbox to active/deactive the alarm
...
const toggleAlarm = (alarm) =>{
    ...
    if (alarm.isActive) {
        const currentTime = new Date().toLocaleTimeString("vi-VN", { hour12: false }).slice(0, 5);
...
//Event listeners for setting up a new alarm
setAlarm.addEventListener("click", ()=>{
    ...
    //Check if the alarm with the same time already exists
